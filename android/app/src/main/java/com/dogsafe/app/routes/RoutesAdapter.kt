...
class RoutesAdapter(
    ...
    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        val visibilityBtn: TextView   = view.findViewById(R.id.visibilityBtn)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.routeRestrictions.text =
            "${route.restrictionCount} restriction${if (route.restrictionCount != 1) "s" else ""}"

        // Visibility button
        holder.visibilityBtn.text = if (route.isVisible) "VISIBLE" else "HIDDEN"
        holder.visibilityBtn.setBackgroundResource(
            if (route.isVisible) R.drawable.dot_green else R.drawable.dot_red
        )

        // Safety dot
        holder.safetyDot.setBackgroundResource(when (route.safetyStatus) {
